...
if __name__ == '__main__':
    ...
    checkPoint = tf.train.Checkpoint(gen=gen, dis=dis, genOptim=genOptim, disOptim=disOptim)

    # 我们将重复使用该种子（因此在动画 GIF 中更容易可视化进度）
    seed = tf.random.normal([NUM_GENERATE_EXAMPLE, NOISE_DIM - 10])  # 随机部分
    ctrl = tf.concat([tf.one_hot(range(10), 10), tf.random.normal([NUM_GENERATE_EXAMPLE - 10, 10])], 0)
    ...
    def train_step(images, labels):
        ...
        ctrl = tf.one_hot(tf.random.uniform([BATCH_SIZE], minval=0, maxval=10, dtype=tf.int32), 10)
